...
fn main() {
    ...
    let two = 2;
    let three = 3;
    assert_eq!(42u32 % one, 0);  // iconst.i32 0
    assert_eq!(7u32 % two, 1);   // band_imm x, 1
    assert_eq!(8u32 % two, 0);   // band_imm x, 1
    //assert_eq!(7u128 % 2, 1);
    assert_eq!(7u32 % three, 1); // urem
    assert_eq!(8u32 % three, 2); // urem
}
